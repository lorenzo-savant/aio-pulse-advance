...
const i18n = {
  en: {
    ...
    shareOfMentions: 'Share of Competitor Mentions',
  ...
  it: {
    ...
    shareOfMentions: 'Quota Menzioni Competitor',
  ...
  sv: {
    ...
    shareOfMentions: 'Andel Konkurrentomnämnanden',
    avgPosition: 'Genomsnittlig Position',
      ...
      <table>
        ...
        <tbody>
          ${competitors
            ...
            <tr>
              <td>${esc(c.name)}</td>
              <td>${c.hasData ? c.mentions : t.noData}</td>
              <td>${c.hasData && c.share !== null ? c.share.toFixed(1) + '%' : t.noData}</td>
              <td>${c.hasData && c.avgPosition !== null ? c.avgPosition.toFixed(1) : t.noData}</td>
            </tr>
          `,
            )
            .join('')}
...
export async function GET(req: NextRequest) {
  ...
  const mappedResults = (results || []).map((r: any) => ({
    ...
    // Carried so citationRate can exclude Brave-substituted URLs.
    citation_source: r.citation_source ?? null,
