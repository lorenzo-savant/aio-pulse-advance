    ...
    <div className="mx-auto max-w-3xl space-y-8">
      ...
      {step === 2 && (
        <Card className="space-y-6 p-6">
          ...
          <label className="bg-secondary/50 flex cursor-pointer items-start gap-3 rounded-xl border border-border px-4 py-3">
            <input
              type="checkbox"
              checked={useAi}
              onChange={(e) => setUseAi(e.target.checked)}
              disabled={generatingPrompts}
              className="accent-brand-500 mt-0.5 h-4 w-4 shrink-0 cursor-pointer"
            />
            <span className="flex flex-col">
              ...
              <span className="text-xs text-muted-foreground">
                Adds 5-10 extra prompts generated by Groq AI (in your primary language) on top of
                the template ones. Requires an LLM key configured.
              </span>
            </span>
          </label>
