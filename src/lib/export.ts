...
export function exportAnalysisToCsv(result: AnalysisResult): void {
  const lines = [
    row('AEO Pulse — Analysis Report', today()),
    row(),
...
export function exportBrandReportToCsv(brand: Brand, results: MonitoringResult[]): void {
  const lines = [
    row('AEO Pulse — Brand Report', today()),
...
<html>
<head>
  <title>${escapeHtml(brand.name)} - AEO Pulse Report</title>
  <style>
...
<body>
  ...
  <div class="footer">
    Generated by AEO Pulse • AI Search Visibility Platform
  </div>
